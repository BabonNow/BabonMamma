...
export default function BABON() {
  ...
  return (
    <div className="p-4 grid gap-4 max-w-xl mx-auto">
      ...
      {!submitted && <div className="mt-4"><div id="paypal-container-WU4R32EETBY98"></div></div>}
      {submitted && (
        <>
          <Button onClick={generateBABON}>Launch BABON Kit</Button>
          {businessPlan && <Card><CardContent><h2 className="text-xl font-semibold mb-2">Business Plan</h2><pre>{businessPlan}</pre></CardContent></Card>}
          {brandKit && <Card><CardContent><h2 className="text-xl font-semibold mb-2">Brand Kit</h2><pre>{brandKit}</pre></CardContent></Card>}
          {marketingKit && <Card><CardContent><h2 className="text-xl font-semibold mb-2">Marketing Kit</h2><pre>{marketingKit}</pre></CardContent></Card>}
        </>
      )}
    </div>
  );
